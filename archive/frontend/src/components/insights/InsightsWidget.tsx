...
export function InsightsWidget({ onCreateGoal, onSetBudget }: InsightsWidgetProps) {
  ...
  // Get styling based on insight type - using Slate & Coral soft wash style
    ...
    switch (type) {
      case 'alert':
        return {
          ...
          label: 'Alert',
          labelBg: 'bg-rose-600/10 text-rose-700',
        };
      case 'opportunity':
        return {
          ...
          label: 'Opportunity',
          labelBg: 'bg-sage-500/10 text-sage-700',
        };
      case 'optimization':
        return {
          ...
          label: 'Optimization',
          labelBg: 'bg-slate-500/10 text-slate-700',
        };
      default:
        return {
          bg: 'bg-surface-base',
      ...
      <div className="bg-white rounded-xl shadow border border-slate-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-lg font-semibold tracking-tight text-slate-900">AI Insights</h2>
              <p className="text-xs text-slate-500">Your personalized financial guidance</p>
            </div>
          </div>
          <button
            onClick={handleRegenerate}
            disabled={isRegenerating}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-50 rounded-lg transition disabled:opacity-50"
          >
            <svg
        ...
        <div className="p-4">
          {insights.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-slate-600 mb-2">No insights available</p>
              ...
              <button
                onClick={handleRegenerate}
                disabled={isRegenerating}
                className="px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-medium hover:bg-slate-800 hover:-translate-y-px transition-all shadow-button disabled:opacity-50"
              >
                Generate Insights
              </button>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className={`text-xs font-semibold uppercase tracking-wide px-2 py-0.5 rounded ${styles.labelBg}`}>
                        {styles.label}
                      </span>
                    </div>

                    {/* Title */}
                    <h3 className="font-semibold text-slate-900 mb-1 line-clamp-2 group-hover:text-slate-700 transition">
                      {insight.title}
                    </h3>
